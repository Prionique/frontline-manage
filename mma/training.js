...
document.getElementById("dtt").innerHTML = `${trainingId.slice(
  8,
  10
)}.${trainingId.slice(5, 7)}&nbsp&nbsp<strong>${trainingId.slice(11)}</strong>`;
...
function displayUsers() {
  ...
  const checkedUsersRef = ref(db, `mma/trainings/${trainingId}/present`);
...
document.getElementById("saveBtn").addEventListener("click", () => {
  ...
  Checkboxes.forEach((checkbox) => {
    const promise = get(ref(db, `users/${checkbox.id}`)).then((snapshot) => {
      if (snapshot.val()["dir"].includes("mma")) {
  ...
  Promise.all(promises).then((results) => {
    ...
    set(ref(db, `mma/trainings/${d.get("id")}/present`), ar);
